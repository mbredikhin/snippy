...
func (r *SnippetPostgres) Create(listID int, snippet snippets.Snippet) (int, error) {
	var id int
	query := fmt.Sprintf("INSERT INTO %s (list_id, name, syntax_id, content) VALUES ($1, $2, $3, $4) RETURNING id", snippetsTable)
	row := r.db.QueryRow(query, listID, snippet.Name, snippet.SyntaxID, snippet.Content)
...
func (r *SnippetPostgres) GetAll(userID, listID int) ([]snippets.Snippet, error) {
	...
	query := fmt.Sprintf(`SELECT st.id, st.list_id, st.name, st.syntax_id, st. content 
	FROM %s st 
...
func (r *SnippetPostgres) Update(userID, snippetID int, input snippets.UpdateSnippetInput) error {
	...
	SET name=$1, list_id=$2, syntax_id=$3, content=$4 
	FROM %s lt, %s ut 
	WHERE st.id=$5 AND st.list_id=lt.id AND lt.user_id=$6 AND lt.user_id=ut.id`, snippetsTable, listsTable, usersTable)
	_, err := r.db.Exec(query, input.Name, input.ListID, input.SyntaxID, input.Content, snippetID, userID)
	return err
}
